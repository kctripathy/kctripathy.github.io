    ...
    .then((data) => {
      ...
      data.projects.forEach((project) => {
        ...
        div.innerHTML = `<div class='col-12 m-1'> 
                                <div class='row'>
                                    <div class='col-lg-9'>#${ctr}: <span class='project-title'>${project.name
          }</span></div>
                                    
                                    <div class='col-lg-3 text-secondary'>${project.duration
          }</div>
                                </div>
                            </div>
                            <div class='col-12 m-1'>${project.description}</div>
                            <div class='col-12 m-1'><span class='project-sub-title'>Project for Company: </span>${project.company
          }</div>
...
function fetchGallery() {
  fetch("./assets/data/gallery.json")
    ...
    .then((data) => {
      ...
      const container_nav = document.getElementById("myTab_gallery");
      const container_tabPanel = document.getElementById("myTabContent_gallery");
      container_nav.innerHTML = "";
      container_tabPanel.innerHTML = "";

      const ul = document.getElementById("myTab_gallery");
      const tabContent = document.getElementById("myTabContent_gallery");
...
function readJsonGetSection(file, section) {
  return fetch(`./assets/data/${file}.json`)
    ...
    .catch((error) => {
      document.getElementById("page-title").innerHTML = "Error loading projects.";
      console.error("Error:", error);
    });
}
...
function getInnerArrayValue(arr) {
  var htmlStr = "";
  arr.forEach((a) => {
    htmlStr = htmlStr + `<a class='inner-link' href='${a.url}' target='_blank'>${a.name}</a>`;
  });
...
/* Collected from Chat GPT 
function traverseJson(obj, indent = "") {
  for (const key in obj) {
    const value = obj[key];

    if (Array.isArray(value)) {
      console.log(`${indent}${key}: [Array of length ${value.length}]`);
      value.forEach((item, index) => {
        if (typeof item === "object") {
          console.log(`${indent}  [${index}] (Object):`);
          traverseJson(item, indent + "    ");
        } else {
